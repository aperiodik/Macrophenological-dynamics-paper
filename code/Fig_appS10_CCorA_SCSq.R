# canonical correlation analysis 
# between isomap components Y of consecutive windows CCorA(Y(i),Y(i+1)):
#   * compute structural correlations squared (SCSq)
#   * plot SCSq corrsp. to Y(i) and SCSq corrsp. to Y(i+1) 

# colours
library("yacca")
library("viridis") 
library("RColorBrewer")

# labels for plot
xvtic = c(1,seq(10,nowi, by=13))
xvlab = c("1/1-30/3", #1
          "4/3-1/6", #10= met. spring
          "3/6-31/8", #23=met. summer
          "2/9-30/11", #36=met. autumn
          "2/12-1/3" #49 met. winter
)

###################
# Canonical correlation analysis CCorA(Y(i),Y(i+1))
###################

nocom2 <- 5 # no of isomap components
nowi2 <- nowi-1 # no of windows of total windows

# SCSq corrsp. to Y(i) 
X_structCorrsqWW <- lapply(1:nowi2, function(i){
  # common locations
  idx_mtb = sort(intersect(MTB.list[[i]], MTB.list[[i+1]]))
  # select common locations of Y
  isoW = as.matrix(filter(as.data.frame(dimR.list[[i]][-1,]), MTB.list[[i]] %in% idx_mtb))
  isoW2 = as.matrix(filter(as.data.frame(dimR.list[[i+1]][-1,]), MTB.list[[i+1]] %in% idx_mtb))
  # CCorA(Y(i),Y(i+1))
  cca.fit <- cca(isoW[, 1:nocom2], isoW2[ ,1:nocom2])
  # SCSq corrsp. to Y(i) 
  cancorlist <- as.vector(cca.fit$xstructcorrsq)
})

X_structCorrsqWWm <- matrix(unlist(X_structCorrsqWW),nrow=nowi2,byrow=T)

# SCSq corrsp. to Y(i+1) 
Y_structCorrsqWW <- lapply(1:nowi2, function(i){
  # common locations
  idx_mtb = sort(intersect(MTB.list[[i]], MTB.list[[i+1]]))
  # select common locations of Y
  isoW = as.matrix(filter(as.data.frame(dimR.list[[i]][-1,]), MTB.list[[i]] %in% idx_mtb))
  isoW2 = as.matrix(filter(as.data.frame(dimR.list[[i+1]][-1,]), MTB.list[[i+1]] %in% idx_mtb))
  # CCorA(Y(i),Y(i+1))
  cca.fit <- cca(isoW[, 1:nocom2], isoW2[ ,1:nocom2])
  # SCSq corrsp. to Y(i+1) 
  cancorlist <- as.vector(cca.fit$ystructcorrsq)
})

Y_structCorrsqWWm <- matrix(unlist(Y_structCorrsqWW),nrow=nowi2,byrow=T)

# Plots
for (i in 1:nocom){
  a0<- i 
  a1 <-nocom2*(a0-1)+1
  a2 <-nocom2*a0
  cvi <- c(a1:a2)
  print(cvi)

  ############--------------------------------
  # PLOT SCSq corresp. to Y(i)
  ############--------------------------------
  fina <- paste("figs/Fig16_StructCorrSq_WinWin_Wini",a0,".pdf", sep = "")
  pdf(fina,       # File name
    width = 8, height = 6, # Width and height in inches
    family="Helvetica",        # font type
    pointsize=22,          # text size
    bg = "white",          # Background color
    colormodel = "cmyk",   # Color model (cmyk is required for most publications)
    #paper = "A4"          # Paper size
  )

  par(mar=c(5, 4, 0.5, 0.5)+0.1)#, mgp=c(3, 1, 0))

  ylabv <- paste("SCSq, CV",a0)
  matplot(X_structCorrsqWWm[,cvi], 
        type = "o", # plot type
        lty = 1, # line type
        col=mycol(12)[mycolN],
        lwd = 2,
        pch = c(20),
        cex = 0.5,
        xlab = "", 
        ylab = ylabv,
        xlim = c(0,nowi+10), ylim = c(0,1),
        xaxt = "n", # remove tick labels
        yaxt = "n", # remove tick labels
        las = 2 # x-axis label:vertical, y-axis label:horizontal
  )

  axis(1, 
     at = xvtic, # vector with label location
     las = 2, # vertical labels
     labels = FALSE #xvlab # vector with labels
  )
  # https://www.tenderisthebyte.com/blog/2019/04/25/rotating-axis-labels-in-r/
  # rotate x-axis labels
  text(x = xvtic,
     ## Move labels to just below bottom of chart.
     y = par("usr")[3] - 0.1,
     ## Use names from the data list.
     labels = xvlab,
     ## Change the clipping region.
     xpd = NA,
     ## Rotate the labels by 35 degrees.
     srt = 35,
     ## Adjust the labels to almost 100% right-justified.
     adj = 0.965,
     ## Increase label size.
     cex = 1)

  # x-axis title and distance to labels
  title(xlab = "Time window", line = 4) 

  fylab <- c(0,0.5,1)#c(0.1,0.3,0.5,0.7)
  axis(2, at = fylab , # vector with label location
     las = 2, # vertical labels
     labels = fylab # vector with labels
  )

  legend("topright",
       inset = c(0.01, 0), 
       legend = c(1:nocom),
       col = mycol(12)[mycolN],
       x.intersp = 0.05, # space btw symbol and text
       #lty = c(matrix(1,1,length(rvvec)+1),2),
       #xjust = 1, # justify legend: 0 = left, 0.5 = centre, 1 = right
       yjust = 1, # justify legend: 0 = top, 0.5 = centre, 1 = bottom
       bty = "n", #no box
       lwd = 1,
       lty = c(NA,NA,NA,NA),
       pch = c(16,16,16,16),
       #cex = 1, # font expansion
       title = "iso Y",
       horiz = F, # shape: row = T(rue), col = F(alse)
       #xpd = TRUE # legend outside plot
  )

  dev.off()

  ############--------------------------------
  # PLOT SCSq corresp. to Y(i+1)
  ############--------------------------------
  fina <- paste("figs/Fig16_StructCorrSq_WinWin_Winj",a0,".pdf", sep = "")
  pdf(fina,       # File name
    width = 8, height = 6, # Width and height in inches
    family="Helvetica",        # font tpye
    pointsize=22,          # text size
    bg = "white",          # Background color
    colormodel = "cmyk",   # Color model (cmyk is required for most publications)
    #paper = "A4"          # Paper size
  )

  par(mar=c(5, 4, 0.5, 0.5)+0.1)#, mgp=c(3, 1, 0))

  mycol <-colorRampPalette(brewer.pal(12,"Paired"))
  mycolN <- c(2,4,6,8,9)
  ylabv <- paste("SCSq, CV",a0)
  matplot(Y_structCorrsqWWm[,cvi], 
        type = "o", # plot type
        lty = 1, # line type
        col=mycol(12)[mycolN],
        lwd = 2,
        pch = c(20),
        cex = 0.5,
        xlab = "", 
        ylab = ylabv,
        xlim = c(0,nowi+10), ylim = c(0,1),
        xaxt = "n", # remove tick labels
        yaxt = "n", # remove tick labels
        las = 2 # x-axis label:vertical, y-axis label:horizontal
  )

  axis(1, 
     at = xvtic, # vector with label location
     las = 2, # vertical labels
     labels = FALSE #xvlab # vector with labels
  )
  # https://www.tenderisthebyte.com/blog/2019/04/25/rotating-axis-labels-in-r/
  # rotate x-axis labels
  text(x = xvtic,
     ## Move labels to just below bottom of chart.
     y = par("usr")[3] - 0.1,
     ## Use names from the data list.
     labels = xvlab,
     ## Change the clipping region.
     xpd = NA,
     ## Rotate the labels by 35 degrees.
     srt = 35,
     ## Adjust the labels to almost 100% right-justified.
     adj = 0.965,
     ## Increase label size.
     cex = 1)

  # x-axis title and distance to labels
  title(xlab = "Time window", line = 4) 

  fylab <- c(0,0.5,1)#c(0.1,0.3,0.5,0.7)
  axis(2, at = fylab , # vector with label location
     las = 2, # vertical labels
     labels = fylab # vector with labels
  )

  legend("topright",
       inset = c(0.01, 0), 
       legend = c(1:nocom),
       col = mycol(12)[mycolN],
       x.intersp = 0.05, # space btw symbol and text
       #lty = c(matrix(1,1,length(rvvec)+1),2),
       #xjust = 1, # justify legend: 0 = left, 0.5 = centre, 1 = right
       yjust = 1, # justify legend: 0 = top, 0.5 = centre, 1 = bottom
       bty = "n", #no box
       lwd = 1,
       lty = c(NA,NA,NA,NA),
       pch = c(16,16,16,16),
       #cex = 1, # font expansion
       title = "iso Y",
       horiz = F, # shape: row = T(rue), col = F(alse)
       #xpd = TRUE # legend outside plot
  )

  dev.off()
}
